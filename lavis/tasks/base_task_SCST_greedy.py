        ...
        for i in metric_logger.log_every(range(iters_per_epoch), log_freq, header):
            ...
            with torch.cuda.amp.autocast(enabled=use_amp):
                if epoch < rl_start_epoch:
                    ...
                    logits = output['logits']  # The raw scores for the last token in each sequence
                    ...
                    # Get greedy actions
                    """_, greedy_action = logits.max(dim=-1)
                    ...
                    for img, gt_caption, gen_caption, greedy_cap in zip(samples['image'], ground_truth_captions, generated_captions, greedy_caption):
                        # Compute custom scores
                        average_errors = evaluator.compute_score(to_pil(img), [gt_caption], [gen_caption])
                        greedy_errors = evaluator.compute_score(to_pil(img), [gt_caption] [greedy_cap])
                        custom_scores_list.append(average_errors)
                        custom_scores_list_greedy.append(greedy_errors)

                    custom_scores_array = np.array(custom_scores_list)
                    custom_scores_greedy_array = np.array(custom_scores_list_greedy)

                    rewards = torch.tensor(custom_scores_array).to(logits.device) - torch.tensor(custom_scores_greedy_array).to(logits.device)
