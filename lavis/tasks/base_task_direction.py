...
class BaseTask:
    ...
    def log_reward_correlations(self, individual_rewards, composite_reward):
        composite_reward_np = composite_reward.cpu().numpy()
        for name, reward_tensor in individual_rewards.items():
            individual_reward_np = reward_tensor.cpu().numpy()
            if len(individual_reward_np) > 1 and len(composite_reward_np) > 1:
                correlation, _ = pearsonr(individual_reward_np, composite_reward_np)
            else:
                correlation = 0.0
    ...
    def compute_reward(self, generated_captions: List[str], reference_captions: List[str]) -> torch.Tensor:
        ...
        total_rewards = torch.zeros(len(generated_captions), device=device)
        individual_rewards = {}

        for name, func in RewardRegistry.registry.items():
            RewardRegistry.weights[name] = RewardRegistry.weights[name].to(device)
