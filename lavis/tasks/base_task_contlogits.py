...
def find_object_position_in_caption(obj, caption):
    """
    Finds the position of an object in the caption.

    Args:
    obj (str): The object to find.
    caption (str): The caption string.

    Returns:
    int: The approximate position of the object in the caption, -1 if not found.
    """
    words = caption.split()
    try:
        return words.index(obj) + 1
    except ValueError:
        return -1  # Object not found

def evaluate_caption(caption, reference_captions):
    """
    Evaluate a generated caption using the CIDEr score.

    Args:
    caption (str): The generated caption to evaluate.
    reference_captions (List[str]): The list of reference captions.

    Returns:
    float: The CIDEr score for the generated caption.
    """

    # Prepare the data for CIDEr
    hypo = {0: [caption]}
...
def continue_caption_from_position(model, logits, start_position, t5_tokenizer, max_length=50):
    """
    Continues generating a caption from a specific position in the logits.

    Args:
    model: The language model used for generation.
    logits (Tensor): The logits from the model for the current batch, shaped [batch_size, seq_length, vocab_size].
    start_position (int): The position in the logits from where to start the continuation.
    max_length (int): Maximum length of the continuation.

    Returns:
    str: The generated caption continuation.
    """
    # Select the logits from the start position for all items in the batch
    logits_to_use = logits[start_position:, :]

    # The continuation process can vary depending on the specifics of your model. 
    # Below is a conceptual approach. You might need to adapt it based on your model's API.

    # Convert logits to token probabilities
    probs = torch.nn.functional.softmax(logits_to_use, dim=-1)

    # Sample from these probabilities to get next token IDs
    next_tokens = torch.multinomial(probs.view(-1, probs.size(-1)), 1)

    # Decode the tokens to a continuation
    continuation = t5_tokenizer.batch_decode(next_tokens, skip_special_tokens=True)
...
def perform_mc_roll_outs(model, logits, gen_obj, caption, tokenizer, reference_captions, num_rollouts=5):
    mc_rewards = []
    ...
    for obj in gen_obj:
        ...
        if obj_position != -1:
            ...
            for _ in range(num_rollouts):
                ...
                continued_caption = continue_caption_from_position(model, logits, token_position, tokenizer)
            ...
            with torch.cuda.amp.autocast(enabled=use_amp):
                if epoch < rl_start_epoch:
                    ...
                    for i, caption in enumerate(generated_captions):
                        mc_reward = perform_mc_roll_outs(model, logits[i], gen_obj[i]['objects'], caption, t5_tokenizer, ground_truth_captions[i], num_rollouts=5)
