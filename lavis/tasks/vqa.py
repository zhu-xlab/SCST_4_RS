...
class VQATask(BaseTask):
    ...
    def _report_metrics(self, result_file, split):
        split_name = split
        # Define the path to ground truth annotations
        json_name = 'merged_' + split_name + '.json'
        #vqa_gt_root = os.path.join(registry.get_path("cache_root"), "coco_gt/" + json_name)
        vqa_gt_root = os.path.join('/mnt/SSD2/thomas/LAVIS/', "coco_gt/" + json_name)

        # Load the ground truth and result file
        vqa_gt = json.load(open(vqa_gt_root))
        vqa_result = json.load(open(result_file))

        # Convert annotations in vqa_gt to a dictionary for faster access
        gt_dict = {item['question_id']: item for item in vqa_gt['annotations']}

        # Initialize counters
        correct_answers = 0
        total_answers = 0
        correct_per_qtype = defaultdict(int)
        total_per_qtype = defaultdict(int)

        for pred in vqa_result:
            # Using the question_id to fetch the corresponding ground truth
            corresponding_gt = gt_dict[pred["question_id"]]

            # Check for correct prediction
            if pred['answer'] in corresponding_gt['answers']:
                correct_answers += 1
                correct_per_qtype[corresponding_gt['question_type']] += 1

            total_answers += 1
            total_per_qtype[corresponding_gt['question_type']] += 1

        # Calculate overall accuracy
        overall_acc = correct_answers / total_answers if total_answers > 0 else 0

        # Calculate accuracy per question type
        per_qtype_acc = {}
        for qtype, total in total_per_qtype.items():
            per_qtype_acc[qtype] = correct_per_qtype[qtype] / total if total > 0 else 0

        # Logging the stats
        log_stats = {
            split_name: {
                "overall": overall_acc,
                "perQuestionType": per_qtype_acc
            }
        }
        ...
        vqa_res = log_stats[split_name]
        vqa_res["agg_metrics"] = overall_acc
