...
class BaseTask:
    ...
    def train_rl_step(self, model: Any, samples: Dict[str, Any]) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        num_rewards = sampled_reward.shape[0]

        reward_combinator = RewardCombiner(num_rewards)

        sampled_reward = reward_combinator(sampled_reward)
        greedy_reward = reward_combinator(greedy_reward)
        ...
        entropy_coeff = 0.001  # Adjust this coefficient as needed
    ...
    class RewardCombiner(nn.Module):
        ...
        def forward(self, x):
            return self.combiner(x).unsqueeze(-1)
    ...
    def compute_reward(self, generated_captions: List[str], reference_captions: List[str]) -> torch.Tensor:
        ...
        """
        rewards = RewardRegistry.compute_total_reward(generated_captions, reference_captions, None, None) # self.evolutionary_algorithm.ema_weights
    ...
    def _train_inner_loop(
        ...
        # Initialize reward combiner optimizer
        reward_combiner_optimizer = torch.optim.Adam(self.reward_combiner.parameters(), lr=0.001)
