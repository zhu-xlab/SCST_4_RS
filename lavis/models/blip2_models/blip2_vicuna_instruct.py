...
class Blip2VicunaInstruct(Blip2Base):
    ...
    def generate(
        ...
        for reprompt_iter in range(2):
            if isinstance(prompt, str):
                #current_prompt = ['Correct your mistakes on this caption "' + output_text[i] + '" of this image based on its details: ' for i in range(bs)]
                current_prompt = ['Caption this satellite image as precisely as possible: <IMAGE> ' for i in range(bs)]
            else:
                if reprompt_iter == 0:
                    current_prompt = ['a photo of ' for i in range(bs)]
                else:
