...
class BlipITM(BlipBase):
    ...
    def forward(self, samples, match_head="itm"):
        ...
        # Temperature Scaling (setting a default value, update it as needed)
        self.temp = torch.Tensor([0.1]).to(image.device)
        
        image_embeds = self.visual_encoder.forward_features(image)
        image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(image.device)

        # Feature Normalization for Image
        image_feat = F.normalize(self.vision_proj(image_embeds[:, 0, :]), dim=-1)
        ...
        # Feature Normalization for Text
        ...
        # Positive samples are the ones where the indices match
        pos_samples = torch.eq(idx.unsqueeze(1), idx.unsqueeze(0)).float()

        # Creating labels for cross entropy loss; the labels are simply the indices of the positive samples
        labels = torch.arange(sim_i2t.shape[0]).to(sim_i2t.device)

        # Computing the loss using cross entropy which combines log softmax and negative log likelihood loss
        loss_i2t = F.cross_entropy(sim_i2t, labels)
        loss_t2i = F.cross_entropy(sim_t2i, labels)

        # The final loss is the mean of the two losses
        ...
        itm_labels = torch.ones(idx.size(0), dtype=torch.long).to(image.device)
        loss_itm = F.cross_entropy(itm_logits, itm_labels)

        return {'loss': loss_itm}

    def forward_2(self, samples, match_head="itm"):
        image = samples["image"]
        caption = samples["text_output"]
...
def compute_gradcam(model, visual_input, text_input, tokenized_text, block_num=6):
    ...
    output = model.forward_2({"image": visual_input, "text_output": text_input}, match_head="itm")
    loss = output[:, 1].sum()
