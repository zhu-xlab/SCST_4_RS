...
def interpolate_pos_embed(model, state_dict, interpolation: str = 'bicubic', seq_dim=1):
    ...
    # Now the reshape operation should result in a tensor with the second dimension being 768
    ...
    if pos_emb_tok is not None:
        # Add print statements to check the sizes of pos_emb_tok and pos_emb_img
        print(f"pos_emb_tok size: {pos_emb_tok.size()}")
        print(f"pos_emb_img size: {pos_emb_img.size()}")
        # Initialize a linear layer that transforms from 912 to 768
...
def interpolate_pos_embed(model, state_dict, interpolation: str = 'bicubic', seq_dim=1):
    ...
    pos_emb_img = pos_emb_img.permute(0, 2, 3, 1).reshape(1, grid_size[0] * grid_size[1], -1)[0]
    if pos_emb_tok is not None:
        # Ensure that pos_emb_img has the same size in dimension 1 as pos_emb_tok
        if pos_emb_tok.size(1) != pos_emb_img.size(1):
            pos_emb_img = pos_emb_img[:, :pos_emb_tok.size(1)]
        new_pos_embed = torch.cat([pos_emb_tok, pos_emb_img], dim=0)
    else:
        new_pos_embed = pos_emb_img
    state_dict['positional_embedding'] = new_pos_embed
...
def create_clip_vit_L(img_size=224,use_checkpoint=False,precision="fp16"):
    ...
    # Load the weights from local file instead
    state_dict = torch.load('RemoteCLIP/RemoteCLIP-ViT-L-14.pt', map_location="cpu")    
